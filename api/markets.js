    ...
    const markets = arr
      .filter(m => {
        ...
        const outcomes = m.outcomes ?
          (typeof m.outcomes === 'string' ? JSON.parse(m.outcomes) : m.outcomes) : null;
        if (outcomes && outcomes.length === 2) {
          const o0 = outcomes[0].toLowerCase().trim();
          const o1 = outcomes[1].toLowerCase().trim();
          if (!(o0 === 'yes' && o1 === 'no') && !(o0 === 'no' && o1 === 'yes')) return false;
        }
        return true;
      })
      .map(m => {
        const outcomes = m.outcomes ?
          (typeof m.outcomes === 'string' ? JSON.parse(m.outcomes) : m.outcomes) : ['Yes', 'No'];
        const o0 = outcomes[0].toLowerCase().trim();
        const yesPrice = o0 === 'yes' ? parseFloat(m.outcomePrices[0]) : parseFloat(m.outcomePrices[1]);
        const noPrice = o0 === 'yes' ? parseFloat(m.outcomePrices[1]) : parseFloat(m.outcomePrices[0]);
        return {
          id: m.id,
